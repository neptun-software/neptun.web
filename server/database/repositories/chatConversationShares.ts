...
import { chat_conversation_share, type ChatConversationShareToCreate, type ReadChatConversationShare } from "~/lib/types/database.tables/schema";
...
export const readShareInfo = async (
    ...
    const shareStatus = await db
        ...
        .catch((err) => {
            if (LOG_BACKEND) console.error('Failed to fetch share from database', err);
            return {
                shareExists: false,
                shareIsActive: false,
...
export const readChatConversationShareMessages = async (
    ...
    const chatConversationShareMessages = await db
        .query.chat_conversation_message.findMany({
            with: {
                chat_conversation: {
                    with: {
